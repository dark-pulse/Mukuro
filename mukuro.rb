...
bot.command :hug do |_event, *args|
  ...
  elsif _event.message.mentions[0] == _event.user.mention
    "#{_event.user.mention} cries in loneliness for having no one to hug them... :sob:"
  else
...
bot.command :regain do |event|
  if author == 1
  event.respond "#{event.user.mention} has regained all his previous coins."
  author -= 1
  else
    "You have not used a bank to store all your coins."
  end
end
