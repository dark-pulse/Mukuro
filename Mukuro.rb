...
TOKEN = 'NDI3MjAwNjI5NTA5OTgwMTcx.DddNRw.I2z_M92hEktQI8DiE3vPYAZ0qJk'
PREFIX = 'm.'
...
bot.message(with_text: "#{PREFIX}quote") do |event|
    ...
    load 'quotes.rb'
         event.channel.send_embed do |embed|
               ...
               if bot.user($quotes["#{r_n}"][0]).on($quotes["#{r_n}"][1]).colour.combined == nil
                embed.colour = Discordrb::ColourRGB.new(0xffffff).combined
               else
